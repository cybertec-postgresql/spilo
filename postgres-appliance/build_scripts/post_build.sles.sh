...
# on suse inking entire dir is ont working
rm -rf /etc/service && ln -snf "$RW_DIR/service" /etc/service
ln -sf "$RW_DIR/pam.d-postgresql" /etc/pam.d/postgresql
ln -s "$RW_DIR/postgres.yml" "$PGHOME/postgres.yml"
ln -sf "$RW_DIR/.bash_history" /root/.bash_history
